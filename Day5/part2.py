...
def crate_mover(warehouse, executions, source, destination):
    ...
    for _ in range(executions):
        boxes_to_move.append(warehouse[source].pop())
    for _ in range(len(boxes_to_move)):
        new_box = boxes_to_move.pop()
        warehouse[destination].append(new_box)
